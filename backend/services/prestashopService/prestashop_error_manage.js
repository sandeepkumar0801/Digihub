...
const prestashopService = require("../prestashopService/prestashop_service");
    ...
    switch (search_case) {
                ...
                if (!isEmpty) {
                    ...
                    if (!isEmpty) {
                        ...
                        for (const prestaAccount of result) {
                            let response = await prestashopService.getProducts(req, res, [prestaAccount], exist_cron, fby_id, cron_name, old_cron_id);

                            let alert_response = await fbyService.insertAlert(api_key, response[prestaAccount.domain], channel, prestaAccount.owner_code, cron_name, old_cron_id);
            ...
            if (result.length > 0) {
                ...
                if (!isEmpty) {
                    ...
                    if (!isEmpty) {
                        ...
                        /*prestashop account loop start */
                        for (const prestaAccount of result) {
                            let response = await fbyService.getStockList(api_key, prestaAccount, req, exist_cron, fby_id, cron_name, old_cron_id);
                ...
                for (const product of result) {
                    ...
                    if (result.length > 0) {
                        ...
                        if (result.length > 0) {
                            /*prestashop account loop start */
                            for (const prestaAccount of result) {
                                let response = await prestashopService.updateQuantityPrestashop([product], prestaAccount, fby_id, cron_name, old_cron_id);
                                let alert_response = await fbyService.insertAlert(api_key, response[product.sku], channel, prestaAccount.owner_code, cron_name, old_cron_id);
                                details[product.sku] = {
                                    fby_user_id: fby_id,
                                    result: alert_response
                                }
        ...
        case "get_presta_orders":
            ...
            if (result.length > 0) {
                ...
                if (result.length > 0) {
                    ...
                    // get prestashop_account user details
                    var { variables, result } = await db.execute(
                        db_constants.DB_CONSTANTS_ACCOUNT.GET_SHOPIFY_USER,
                        [fby_id]
                    );
                    ...
                    if (result.length > 0) {
                        ...
                        /*prestashop account loop start */
                        for (const prestaAccount of result) {
                            let response = await prestashopService.getOrders(req, res, [prestaAccount], exist_cron, fby_id, cron_name, old_cron_id);
        ...
        case "push_Track_Presta":
            ...
            if (result.length > 0) {
                ...
                for (const order of result) {
                    ...
                    if (result.length > 0) {
                        ...
                        if (result.length > 0) {
                            /*prestashop account loop start */
                            for (const prestaAccount of result) {
                                let response = await prestashopService.pushTrackingPresta([order], prestaAccount, fby_id, cron_name, old_cron_id);
                                let alert_response = await fbyService.insertAlert(api_key, response[order.order_no], channel, prestaAccount.owner_code, cron_name, old_cron_id);
                                details[order.sku] = {
                                    fby_user_id: fby_id,
                                    result: alert_response
                                }
                ...
                let message = {
                    case: search_case,
                    query_action: "Get tracked order details",
                };
                helpers.sendError(
                    res,
                    constants.HTTPSTATUSCODES.NOT_FOUND,
