...
const adapter = require("../../services/storeden_service");
...
exports.getStoredenProducts = async (req, res) => {
  let cron_name = constants.CC_OPERATIONS.GET_PRODUCT_FROM_CHANNEL;
  let file_and_method_name = 'storedenController.js getStoredenProducts';
  let cron_id = uuid();
      ...
      common.userDetail(fby_user_id, cron_name, cron_id, function (result) {
        if (result.error) {
          ...
          for (const user of result.success.data) {
            let storeden_id = user.fby_user_id;
            //get shopify account detail
            common.shopifyUserDetail(storeden_id, cron_name, cron_id, function (result) {
              if (result.error) {
                // store shopify account error log
                let inputs = [cron_name, cron_id, constants.QUERY_TYPE, JSON.stringify(result.error), storeden_id];
                common.cronErrorLog(inputs, cron_name, cron_id, function (result) {
                  if (result.error) {
                    mail.cronLogErrMail(cron_name, cron_id, storeden_id, JSON.stringify(result.error));
                  }
                });
                //send response
                res.send(result.error);
                ...
                if (true) {
                  ...
                  let exist_cron = 0;
                  common.getBulkCronLog(storeden_id, dt.format('Y-m-d'), cron_name, new_cron_id, function (result) {
                    if (result.success) {
                    ...
                    adapter.getProducts(shopifyAccount, exist_cron, storeden_id, cron_name, new_cron_id)
                      .then((params) => {
                        res.send(params);
                      });
                  });
...
exports.sendProductsStoreden = async (req, res) => {
  let cron_name = constants.CC_OPERATIONS.PUSH_PRODUCTS_TO_CHANNEL;
  let file_and_method_name = 'storedenController.js sendProductsStoreden';
  let cron_id = uuid();
      ...
      await common.userDetail(req.query.fby_user_id, cron_name, cron_id, async function (result) {
        if (result.error) {
          ...
          for await (const user of result.success.data) {
            ...
            */
            if (user[0].productPublish == 1) {
              await common.getProduct(user, "status", cron_name, cron_id, async function (result) {
                if (result.error) {
                  ...
                  for (const product of result.success.data) {
                    ...
                    await adapter.insertSku(product, exist_cron, cron_name, new_cron_id, function (result) {
              ...
              set_response = {};
              set_response[user[0].domain] = constants.CC_STOCK_UPDATE_NOT_ALLOWED_MSG;
              if (!res.headersSent) {
                res.send(set_response);
              }
            }
...
exports.getStoredenStock = async (req, res) => {
  let cron_name = "get_Storeden_Stock";
  let file_and_method_name = 'storedenController.js getStoredenStock';
  let cron_id = uuid();
  let fby_user_id = req.query.fby_user_id;
  let cacheKey_Job = `${cron_name}_fby_user_id_${fby_user_id}`;
...
exports.pushStockStoreden = async (req, res) => {
  let cron_name = constants.CC_OPERATIONS.PUSH_STOCK_TO_CHANNEL;
  let file_and_method_name = 'storedenController.js pushStockStoreden';
  let cron_id = uuid();
  let fby_user_id = req.query.fby_user_id;
  let cacheKey_Job = `${cron_name}_fby_user_id_${fby_user_id}`;
  try {
            ...
            common.shopifyUserDetail(fby_id, cron_name, cron_id, function (result) {
              if (result.error) {
                ...
                for (const shopifyAccount of result.success.data) {
                  ...
                  */
                  if (shopifyAccount.stockUpdate == 1) {
                    common.getProduct(shopifyAccount, "domain", cron_name, cron_id, function (result) {
                      if (result.error) {
                        // store get product error log
                        let inputs = [cron_name, cron_id, constants.QUERY_TYPE, JSON.stringify(result.error), fby_id];
                        common.cronErrorLog(inputs, cron_name, cron_id, function (result) {
                        ...
                        adapter.pushProductsStoreden(result.success.data, shopifyAccount, cron_name, cron_id)
                          .then((params) => {
...
exports.getStoredenOrders = async (req, res) => {
  let cron_name = constants.CC_OPERATIONS.GET_ORDER_FROM_CHANNEL;
  let file_and_method_name = 'storedenController.js getStoredenOrders';
  let cron_id = uuid();
  let fby_user_id = req.query.fby_user_id;
  let cacheKey_Job = `${cron_name}_fby_user_id_${fby_user_id}`;
  try {
            ...
            common.shopifyUserDetail(fby_id, cron_name, cron_id, function (result) {
              if (result.error) {
                ...
                if (shopifyAccount[0].orderSync == 1) {
                  ...
                  set_response = {};
                  set_response[shopifyAccount[0].domain] = constants.CC_ORDER_UPDATE_NOT_ALLOWED_MSG;
                  if (!res.headersSent) {
                    res.send(set_response);
                  }
                }
...
exports.getStoredenTrackNumber = async (req, res) => {
  let cron_name = "get_track_number_storeden";
  let file_and_method_name = 'storedenController.js getStoredenTrackNumber';
  let cron_id = uuid();
  let fby_user_id = req.query.fby_user_id;
  let cacheKey_Job = `${cron_name}_fby_user_id_${fby_user_id}`;
  try {
      ...
      common.userDetail(req.query.fby_user_id, cron_name, cron_id, function (result) {
        if (result.error) {
          ...
          for (const user of result.success.data) {
            ...
            //} else {
            //let api_key = result.success.data;
            common.getUntrackOrders(fby_id, channelName, cron_name, cron_id, function (result) {
              if (result.error) {
                // store user error log
                let inputs = [cron_name, cron_id, constants.QUERY_TYPE, JSON.stringify(result.error), fby_id];
                common.cronErrorLog(inputs, cron_name, cron_id, function (result) {
...
exports.pushTrackStoreden = async (req, res) => {
  let cron_name = constants.CC_OPERATIONS.GET_TRAKING_FROM_FBY;
  let file_and_method_name = 'storedenController.js pushTrackStoreden';
  let cron_id = uuid();
  let fby_user_id = req.query.fby_user_id;
  let cacheKey_Job = `${cron_name}_fby_user_id_${fby_user_id}`;
  try {
                ...
                for (const shopifyAccount of result.success.data) {
                  ...
                  */
                  if (shopifyAccount.orderSync == 1) {
                    common.getOrder(shopifyAccount, "account", cron_name, cron_id, function (result) {
                      if (result.error) {
                        // store get product error log
                        let inputs = [cron_name, cron_id, constants.QUERY_TYPE, JSON.stringify(result.error), fby_id];
                        common.cronErrorLog(inputs, cron_name, cron_id, function (result) {
                        ...
                        //asynchronous function for updating shopify inventory
                        adapter.pushTrackingStoreden(result.success.data, shopifyAccount, cron_name, cron_id, req, res)
                          .then((params) => {
